...
export const WorldMap = () => {
  ...
  const [bengaluruX, bengaluruY] = projection(
    bengaluruCoords as [number, number],
  ) || [0, 0];
    ...
    <div className="world-map-wrapper">
      <div className="world-map-svg-container">
        ...
        >
          ...
          <g>
            {(worldData as unknown as WorldData).features.map(
              (feature: GeoFeature, i: number) => {
          ...
          {/* City Marker Dots */}
          {cities.map((city, i) => {
            const [x, y] = projection(city.coords as [number, number]) || [
              0, 0,
            ];
            return (
              <motion.circle
        ...
        >
          ...
          <div className="location-card-timezone">GMT +5:30</div>
        </motion.div>
      </div>
    </div>
  );
